...
{ 
    ...
    for (i = A; i != Z;  i++){
	    ...
	    for (int j = 0; j < this_i_len-2; j++){
	    	
		this_char = this_i.at(j);
		
	    	this_char_1 = this_i.at(j+1);
	    	
		this_char_2 = this_i.at(j+2);
		...
		for (std::vector<char_count>::const_iterator k = alminac.begin(); k != alminac.end(); k++){
			...
			alminac_member = *k;
			if(alminac_member.specific_char == this_char && alminac_member.specific_char_1 == this_char_1 && alminac_member.specific_char_2 == this_char_2){	
		...
		if(viable_candidate == true){
			...
			new_char.specific_char_2 = this_char_2;
	...
	myfile<< alminac_member.specific_char_1; 
	myfile<< alminac_member.specific_char_2<<endl; 
